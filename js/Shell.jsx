    ...
    <div style={{minHeight:'100%',background:'var(--surface-page)',display:'flex',flexDirection:'column'}}>
      <NavBar items={['writing','projects','about']} active={route==='post'?'writing':route} onNavigate={go}
        right={<>
          ...
          <Tooltip label="github"><IconButton name="github" label="GitHub" size="sm" variant="ghost" onClick={()=>window.open('https://github.com/rivernate','_blank')}/></Tooltip>
          <Tooltip label="rss"><IconButton name="rss" label="RSS" size="sm" variant="ghost" onClick={()=>{window.location.href='/feed.xml'}}/></Tooltip>
        </>}/>
